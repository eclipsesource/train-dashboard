...
public class GermanyMap extends Composite {
  ...
  private final MarkerPool markerPool = new MarkerPool();
  ...
  public void setInfo( RailwayInfo info ) {
    ...
    List<Station> stationsSortedByDelayMinutes = info.getDelayInfo()
      .getStationsSortedByDelayMinutes( 15 );
  ...
  public void addMarker( double longitude, double latitude, final Station station ) {
    Point location = CoordinatesUtil.transformToPoint( new Coordinates( longitude, latitude ),
                                                       this.getSize().x,
                                                       this.getSize().y,
                                                       new Coordinates( 54.8, 6.0 ),
                                                       new Coordinates( 47.3, 14.7 ) );
    ...
    marker.addListener( SWT.MouseDown, new Listener() {
      ...
      public void handleEvent( Event event ) {
        ...
        Shell dialog = new Shell( getDisplay(), SWT.CLOSE
                                                | SWT.BORDER
                                                | SWT.TITLE
                                                | SWT.APPLICATION_MODAL );
        org.eclipse.swt.graphics.Point touchPoint = getDisplay().map( ( Control )event.widget,
                                                                      null,
                                                                      event.x,
                                                                      event.y );
